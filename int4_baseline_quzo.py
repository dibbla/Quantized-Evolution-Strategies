...
def parse_args():
    ...
    parser.add_argument("--worker_extn_cls", type=str,
                        default="utils_int4.worker_extn_quzo_baseline.BaselineQuZOExtension",
...
def launch_int4_engines(num_engines, model_name, worker_extn_cls="utils_int4.worker_extn_quzo_baseline.BaselineQuZOExtension", **kwargs):
...
def main(args):
    ...
    for i in range(args.num_iterations):
        ...
        effective_gain = float(args.discrete_step_gain) * 0.5 

        per_seed_coeffs = [
            (int(seed), float(seeds_perf[int(seed)]["norm_reward"]) * effective_gain)
            for seed in seeds
        ] # compensate for SPSA

        update_step_seed = (args.global_seed or 42) + i + 99999
        update_start = time.time()
        upd_res = ray.get(engines[0].collective_rpc.remote(
            "apply_quzo_perturb_update",
            args=(per_seed_coeffs, args.sigma, float(alpha_t), update_step_seed)
