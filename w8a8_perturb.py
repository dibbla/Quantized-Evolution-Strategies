# ES fine-tuning for W8A8 quantized models (QuZO perturbation + discrete ES update)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ES Fine-tuning for Countdown Task with static scheduling (W8A8)"
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default="neuralmagic/Qwen2.5-0.5B-Instruct-quantized.w8a8",
    )
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "For --reward_weighting rank: keep only the top elite_frac seeds "
            "(0<elite_frac<=1) and zero out others. Use 1.0 to use all seeds."
        ),
    )

    # Discrete update knobs
    parser.add_argument(
        "--save_residual",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable residual accumulation for discrete ES updates (default: enabled; use --no-save-residual to disable)",
    )
    parser.add_argument(
        "--momentum",
        type=float,
        default=0.9,
        help="Residual decay (momentum) for discrete ES updates",
    )
    parser.add_argument(
        "--discrete_step_gain",
        type=float,
    ...
    parser.add_argument(
        "--worker_extn_cls",
        type=str,
        default="utils_w8a8.worker_extn_w8a8_seed_replay.W8A8SeedReplayESExtension",
...
def _assign_rank_weights(
    ...
    denom = max(1, n - 1)
    weights = 1.0 - 2.0 * (ranks.astype(np.float64) / float(denom))
...
def launch_w8a8_engines(
    ...
    print(f"Launched {len(engines)} W8A8 QuZO engines with worker extension {worker_extn_cls}")
    return engines, pgs
...
def main(args):
    ...
    with open("data/countdown.json", "r") as f:
        task_datas = json.load(f)
    task_datas = task_datas[:200]

    eval_task_datas: List[Dict[str, Any]] = []
    ...
    engines, pgs = launch_w8a8_engines(args.num_engines, args.model_name, worker_extn_cls=args.worker_extn_cls)
    ...
    for i in range(args.num_iterations):
        print(f"\n\n=== Generation {i} (static scheduling, W8A8) ===")
        total_iter_start = time.time()

        alpha_t = _alpha_for_iteration(i, args)
        writer.add_scalar("opt/alpha", float(alpha_t), i)

        iter_weighted_wall = {"perturb": 0.0, "infer": 0.0, "restore": 0.0}
            ...
            ray.get(
                [
                    engines[eng_idx].collective_rpc.remote(
                        ...
                        kwargs={"seed": int(seed), "sigma": args.sigma},
                    )
        ...
        if args.mirror_sampling:
            base_seeds = [abs(int(s)) for s in seeds[::2]]
            seen = set()
            base_seeds_ordered = []
            for s in base_seeds:
                if s not in seen:
            ...
            for s in base_seeds_ordered:
                ...
                if sp not in seeds_perf or sn not in seeds_perf:
                    raise RuntimeError(
                        f"Mirror sampling missing pair evaluations for seed {s}: have +? {sp in seeds_perf}, -? {sn in seeds_perf}"
                    )
        ...
        per_seed_coeffs = [
            (
                int(seed),
                float(seeds_perf[int(seed)]["norm_reward"]) * float(args.discrete_step_gain),
            )
            for seed in seeds
        ]
        ...
        upd_res = ray.get(
            engines[0].collective_rpc.remote(
                ...
                kwargs={
                    "keep_residual": bool(args.save_residual),
                    "residual_decay": float(args.momentum),
                },
            )
        )[0]
        update_elapsed = time.time() - update_start
        if args.verbose:
            print(f"Applied updates in {update_elapsed:.3f}s; res={upd_res}")
        ...
        if (args.eval_interval > 0 and (i % args.eval_interval == 0)) or (
